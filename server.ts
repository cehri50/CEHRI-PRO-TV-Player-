...
async function startServer() {
  ...
  // This is critical for IPTV because many Xtream/M3U panels use self-signed or expired SSL certificates.
  ...
  app.get('/api/m3u/proxy', async (req, res) => {
    let m3uUrl = req.query.url as string;
    if (!m3uUrl) {
    ...
    try {
      ...
      if (isHtml && isGoogleDrive) {
        ...
        if (confirmMatch && fileId) {
          ...
          if (gdResponse.ok) {
            ...
            // Update isHtml for the new content to ensure it's not still HTML
      ...
      // If the final content is still HTML (even after attempting confirmation or if there was no confirm match)
      if (isHtml) {
        if (isGoogleDrive) {
          if (content.includes('not found') || content.includes('Bulunamadı') || content.includes('görünüşe göre bu dosya yok') || content.includes('görünmüyor') || content.includes('Bulunamıyor') || content.includes('Bulunamadı')) {
            throw new Error('Google Drive dosyası bulunamadı! Lütfen dosya ID\'sinin doğru olduğundan ve dosyanın silinmediğinden emin olun.');
          }
          if (content.includes('denied') || content.includes('yetkiniz yok') || content.includes('erişim') || content.includes('access') || content.includes('accounts.google.com') || content.includes('ServiceLogin') || content.includes('signin')) {
            throw new Error('Google Drive dosyanız "Kısıtlı" (Özel) veya korumalı durumdadır. Lütfen Google Drive üzerinden paylaşım ayarını "Bağlantıya sahip olan herkes görüntüleyebilir" olarak değiştirin.');
          }
          throw new Error('Google Drive geçerli bir M3U dosyası yerine bir HTML web sayfası döndürdü. Lütfen dosyanın herkese açık paylaşıldığından emin olun.');
        } else {
          throw new Error('Sağlayıcı M3U dosyası yerine bir HTML web sayfası döndürdü. Girdiğiniz URL doğrudan canlı liste indirme bağlantısı olmalıdır, bir web sitesi veya portal adresi değil.');
        }
      }
  ...
  const handleXtreamProxy = async (req: any, res: any) => {
    ...
    // Normalize serverUrl
    let normalizedUrl = (serverUrl as string).trim();
